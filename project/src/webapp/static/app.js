...
async function withSpinner(btn, fn) {
  ...
  btn.disabled = true;
  btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin mr-2"></i>Runningâ€¦';
  try {
    await fn();
  } finally {
    btn.innerHTML = orig;
...
if (analyzeBtn) {
  analyzeBtn.addEventListener('click', async () => {
    ...
    await withSpinner(analyzeBtn, async () => {
      showStatus(statusEl, 'Grouping parties and geolocating across all case filesâ€¦', 'info');
...
if (runDeepBtn) {
  runDeepBtn.addEventListener('click', async () => {
    ...
    await withSpinner(runDeepBtn, async () => {
      showStatus(statusEl, 'Extracting timelinesâ€¦', 'info');
